...
class BasicSimulation extends Simulation {
  ...
  val httpProtocol = http
    .baseUrl("https://ruby-fuse-demo-papi-ruby-fuse-ft-a.app3.osp.pccw.com/fusedemo") // Here is the root for all relative URLs
  ...
  // MuleSoft VPC test
  val scn_vpc = scenario("Scenario #1").repeat(100) {  // millisecond  .repeat(100)  during(1000 * 60)
  ...
  val scn_sfdc_contacts = scenario("Scenario #2").repeat(1) {
    exec(http("Get Salesforce contacts")
      .get("/customers/salesforce-contacts").check(status is 200)
    )
  }


  //setUp(scn_sfdc_contacts.inject(atOnceUsers(1)).protocols(httpProtocol))

  setUp(scn_sfdc_contacts.inject(atOnceUsers(20), constantUsersPerSec(20).during(120.seconds)).protocols(httpProtocol))
